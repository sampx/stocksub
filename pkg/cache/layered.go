...
type LayerFactory interface {
	...
	CreateLayer(config LayerConfig, layerIndex int) (Cache, error)
...
type LayeredCacheStats struct {
	LayerStats   []CacheStats `json:"layer_stats"`
...
func NewLayeredCacheWithFactories(config LayeredCacheConfig, customFactories map[LayerType]LayerFactory) (*LayeredCache, error) {
	layers := make([]Cache, 0, len(config.Layers))
	...
	lc := &LayeredCache{
		...
		factories: factories,
		stats: LayeredCacheStats{
			LayerStats: make([]CacheStats, len(layers)),
		},
...
func createCacheLayer(config LayerConfig, layerIndex int, factories map[LayerType]LayerFactory) (Cache, error) {
...
func (lc *LayeredCache) Get(ctx context.Context, key string) (interface{}, error) {
	...
	return nil, NewCacheError(ErrCacheMiss, "cache miss")
}
...
func (lc *LayeredCache) Clear(ctx context.Context) error {
	...
	lc.stats = LayeredCacheStats{
		LayerStats: make([]CacheStats, len(lc.layers)),
	}
...
func (lc *LayeredCache) BatchGet(ctx context.Context, keys []string) (map[string]any, error) {
	...
	for i, layer := range lc.layers {
		...
		// 检查当前层是否支持批量操作
		if batchGetter, ok := layer.(BatchGetter); ok {
			...
			for _, key := range remainingKeys {
				...
				if err == nil {
					...
					var cacheErr *CacheError
					if !errors.As(err, &cacheErr) || cacheErr.Code != ErrCacheMiss {
...
func (lc *LayeredCache) BatchSet(ctx context.Context, items map[string]any, ttl time.Duration) error {
	...
	if lc.config.WriteThrough {
		...
		var lastErr error
		for i, layer := range lc.layers {
			if batchSetter, ok := layer.(BatchSetter); ok {
...
func (f *memoryLayerFactory) CreateLayer(config LayerConfig, layerIndex int) (Cache, error) {
...
func (f *diskLayerFactory) CreateLayer(config LayerConfig, layerIndex int) (Cache, error) {
...
func (f *remoteLayerFactory) CreateLayer(config LayerConfig, layerIndex int) (Cache, error) {
