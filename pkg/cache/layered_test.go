...
func (m *mockLayer) Get(ctx context.Context, key string) (interface{}, error) {
	...
	m.stats.MissCount++
	return nil, NewCacheError(ErrCacheMiss, "not found")
}
...
func (m *mockLayer) Stats() CacheStats {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.stats
}
...
func (f *mockFactory) CreateLayer(config LayerConfig, layerIndex int) (Cache, error) {
	return f.layer, nil
}
