...
// CSVStorage 实现了 Storage 接口，提供了将测试数据以CSV格式持久化到磁盘的功能。
// 它支持按日期和类型自动分割文件，并利用资源池来提高性能。
type CSVStorage struct {
	config      CSVStorageConfig
	resourceMgr *ResourceManager
	fileMgr     *FileManager
	writerCache map[string]*CSVWriterWrapper
	mu          sync.RWMutex
...
type CSVStorageConfig struct {
	...
	ResourceConfig ResourceConfig `yaml:"resource_config"` // 底层资源管理器（如缓冲区、写入器）的配置。
}
...
type CSVStorageStats struct {
	...
	ResourceStats ResourceStats `json:"resource_stats"` // 底层资源的统计信息。
...
func NewCSVStorage(config CSVStorageConfig) (*CSVStorage, error) {
	...
	resourceMgr := NewResourceManager(config.ResourceConfig)
	fileMgr := NewFileManager(resourceMgr)

	storage := &CSVStorage{
		config:      config,
		resourceMgr: resourceMgr,
		fileMgr:     fileMgr,
		writerCache: make(map[string]*CSVWriterWrapper),
...
func (cs *CSVStorage) Save(ctx context.Context, data interface{}) error {
	...
	if strings.HasPrefix(record.Type, "structured_") {
		if sd, ok := data.(*StructuredData); ok {
...
func (cs *CSVStorage) BatchSave(ctx context.Context, dataList []interface{}) error {
	...
	structuredDataSchemas := make(map[string]*DataSchema) // 存储每个组的schema
...
func (cs *CSVStorage) Close() error {
	...
	cs.writerCache = make(map[string]*CSVWriterWrapper)
...
func (cs *CSVStorage) getOrCreateWriter(recordType, date string) (*CSVWriterWrapper, error) {
	...
	writer := NewCSVWriterWrapper(file, cs.resourceMgr)
...
func (cs *CSVStorage) getStructuredDataCSVHeaders(schema *DataSchema) []string {
...
func (cs *CSVStorage) writeStructuredDataHeader(writer *CSVWriterWrapper, schema *DataSchema) error {
...
func (cs *CSVStorage) ensureStructuredDataHeader(writer *CSVWriterWrapper, recordType, date string, schema *DataSchema) error {
	// 检查文件是否为空（需要写入表头）
	filename := fmt.Sprintf("%s_%s_%s.csv", cs.config.FilePrefix, recordType, date)
	filepath := filepath.Join(cs.config.Directory, filename)
...
// generateStructuredDataValue 将结构化数据格式化为CSV记录中的一行
// 该方法会遍历结构化数据的所有字段，并根据字段类型进行相应的格式化处理
//
// 参数:
//   - sd: 指向结构化数据的指针，包含要格式化的数据及其模式定义
//
// 返回值:
//   - []string: 格式化后的CSV记录，每个元素对应一个字段的字符串表示
//
// 特殊处理:
//   - 时区处理：使用上海时区(Asia/Shanghai)进行时间格式化，如果加载失败则使用UTC+8
//   - 错误处理：如果字段获取失败或值为nil，则使用空字符串表示
func (cs *CSVStorage) generateStructuredDataCSVRecord(sd *StructuredData) []string {
...
// formatStructuredDataValue 将结构化数据值格式化为字符串表示
//
// 参数:
//
//	value: 需要格式化的值，可以是任意类型
//	fieldType: 值的字段类型，决定了格式化的方式
//	timezone: 时区信息，用于时间类型的格式化
//
// 返回值:
//
//	string: 格式化后的字符串表示
...
//   - nil: 返回空字符串
//   - string: 直接返回原字符串
//   - int/int32/int64: 格式化为十进制数字符串
//   - float32/float64: 格式化为保留两位小数的字符串
//   - bool: 返回"true"或"false"
//   - time.Time: 使用指定时区格式化为"YYYY-MM-DD HH:mm:ss"
//   - 其他类型: 使用fmt.Sprintf默认格式化
func (cs *CSVStorage) formatStructuredDataValue(value interface{}, fieldType FieldType, timezone *time.Location) string {
	if value == nil {
		return ""
	}
